...
<div class="body">
	...
	<col />
	  ...
	  <tr class="title"> 
		...
		<td><select name="course.categoryId" id="categoryId">
		<option value="">(请选择...)</option>
		<#list types as tp>
		<option value="${tp.id?if_exists?html}">${tp.name?if_exists?html}</option>
		</#list>
		</select>
		<select name="course.subCategoryId" id="subCategoryId1" class="_categoryId">
		<option value="">(请选择...)</option>
		</select>
		
		</td>
		<td >序号</td>
		<td ><input type="text" name="course.indexNo" id="course.indexNo" size="10" maxlength="40" class="numberEdit" value="0"/>

<label><input type="checkbox" name="course.showInWeb" value="true" id="course.showInWebChk"/> 显示在网站上 </label>
</td>
		</tr>
...
<script type="text/javascript" src="${base}/static/app/javascript/course.js"></script>
...
var lang = {
	name:'名称',no:'编号',domain:'课程',expiredTime:'失效日期',publishTime:'发布日期',showInWeb:'显示在网站上',indexNo:'序号',
	categoryName:'分类',status:'状态',subCategoryName:'子分类'
}
	...
	function check(){
		...
		if(isEmpty($F("categoryId"))){
			alert("请选择分类");
			$("categoryId").focus();
			return false; 
		}
		if(isEmpty($F("subCategoryId1"))){
			alert("请选择分类");
			$("subCategoryId1").focus();
			return false; 
		}
