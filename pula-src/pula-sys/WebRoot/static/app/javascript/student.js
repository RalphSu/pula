var PPage = new Class({
	...
	assign: function(id){
		...
		$("spnStudentNo").set('html',matchNodes[0].no ) ;
		$("spnStudentName").set('html',matchNodes[0].name ) ;
	...
	createPage : function(b){
        ...
        var url = encodeURIComponent('../student/create');
        centerWindow('../my/window?u=' + url,850, 480);
	},
	updatePage : function(id ){
		var url = encodeURIComponent('../student/update?id='+id);
		        centerWindow('../my/window?u=' + url,850, 480);
	},view : function(id ){
		var url = encodeURIComponent('../student/view?id='+id);
