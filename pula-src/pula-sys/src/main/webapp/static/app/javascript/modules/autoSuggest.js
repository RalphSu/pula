...
var autoSuggest = new Class({
	...
	*/
	addSuggestion: function(displayValue,hiddenValue){
		var suggestion = new Element('li', {
			'html': this.buildItem ( displayValue ) ,// this.markupSuggestion(displayValue),
		...
		var displayValue = new Element('input',{
			'type':'hidden',
			'value':JSON.encode(displayValue)
		});			
	...
	buildItem : function( s ) {
		var v = "<h1>{no}</h1><h2>{name}</h2><div class='c'></div>";
		//alert(s);
		return  v.substitute({ 'no':this.markupSuggestion(s[0]) ,
				'name':this.markupSuggestion(s[1])} );
	...
	*/
	handleUserChoice: function(item){
		...
		//alert( item.getChildren('input')[1].value);
		if(this.callback){
			this.callback ( item.getChildren('input')[1].value ) ;
		}else{
